#!/usr/bin/env python3
"""
thermal_blob_detector_mvp.py

Standalone MVP v2 for detecting and tracking bright moving blobs in thermal video.

Scope:
- Detect small bright moving objects on a mostly static thermal background.
- Track centroid movement across frames.
- Export debug video and CSV track points.
- No line/AOI counting logic is included here.

Typical use:
    python thermal_blob_detector_mvp_v2.py --input sample.mp4 --output debug.mp4 --csv track_points.csv --show

This is intended as a future detector backend for ComputerVisionCounter-style workflows,
where line/AOI counting already exists elsewhere.
"""
...
class BlobDetection:
    ...
    def as_cvc_detection(self) -> dict:
        """
        Adapter-style dict that can later be mapped into CVC detection objects.
        BBox is returned as x1,y1,x2,y2 because most detection pipelines prefer that.
        """
        x, y, w, h = self.bbox
        cx, cy = self.centroid
        return {
...
class Track:
    ...
    def predicted_point_for_frame(self, frame_idx: int) -> Optional[Point]:
        """
        Predict next centroid using the last observed velocity.

        This is intentionally simple. It helps when bright/near objects move fast
        and jump more than max_link_distance between frames. If the track has only
        one point, the last point is used.
        """
        if not self.detections:
            return None
...
class ThermalBlobConfig:
    ...
    # Fast bright objects may be visible for only 1-2 frames. Drawing unconfirmed
    # tracks helps diagnose whether they are detected but not yet confirmed.
    draw_unconfirmed_tracks: bool = True
    ...
    # Optional rectangular processing ROI: x,y,w,h.
    roi: Optional[Tuple[int, int, int, int]] = None

    # Debug drawing.
    draw_all_candidates: bool = False
    draw_inactive_tracks: bool = True
    trail_length: int = 9999
...
class ThermalBlobDetector:
    ...
    For CVC integration, detections can be converted using:
...
def draw_debug_overlay(
    ...
    The detections and frame_idx arguments are kept so this function can keep the
    same call signature as the earlier diagnostic version.
    """
    out = frame.copy()
    if out.ndim == 2:
    ...
    for track in detector.tracks.values():
        ...
        is_confirmed = track.lifetime >= cfg.min_track_lifetime
        if not is_confirmed and not cfg.draw_unconfirmed_tracks:
            continue
        ...
        color = color_for_track(track.track_id) if is_confirmed else (180, 180, 180)
...
def write_track_points_csv(path: Path, tracks: Dict[int, Track], fps: float, min_lifetime: int) -> None:
    rows = []
    for track in tracks.values():
        confirmed = int(track.lifetime >= min_lifetime)
...
def parse_roi(value: Optional[str]) -> Optional[Tuple[int, int, int, int]]:
    ...
    if len(parts) != 4:
        raise argparse.ArgumentTypeError("ROI must use format x,y,w,h")
...
def process_video(args: argparse.Namespace) -> None:
    ...
    cfg = ThermalBlobConfig(
        ...
        draw_unconfirmed_tracks=not args.hide_unconfirmed_tracks,
        ...
        draw_all_candidates=args.draw_all_candidates,
    ...
    try:
        while True:
            ...
            if args.show:
                cv2.imshow("Thermal blob detector MVP", debug_frame)
                key = cv2.waitKey(1) & 0xFF
                if key in (27, ord("q")):
                    print("Stopped by user.")
                    break

            if args.max_frames and frame_idx >= args.max_frames - 1:
                break
    ...
    if csv_path:
        write_track_points_csv(csv_path, detector.tracks, fps, cfg.min_track_lifetime)
    ...
    print("Done.")
    print(f"Frames processed: {frame_idx + 1}")
    print(f"All tracks: {len(all_tracks)}")
    print(f"Confirmed tracks >= {cfg.min_track_lifetime} frames: {len(confirmed)}")


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Detect and track bright moving blobs in tripod thermal video. No line/AOI counting included."
    )
    parser.add_argument("--input", required=True, help="Input thermal video file")
    parser.add_argument("--output", default="thermal_blob_debug.mp4", help="Output debug video path")
    parser.add_argument("--csv", default="thermal_blob_tracks.csv", help="Output CSV path for track points")
    ...
    parser.add_argument("--min-track-lifetime", type=int, default=3, help="Minimum detections before a track is treated as confirmed")
    ...
    parser.add_argument("--draw-all-candidates", action="store_true", help="Draw all raw accepted blob detections, not only tracks")
    parser.add_argument("--hide-unconfirmed-tracks", action="store_true", help="Hide temporary tracks shorter than min-track-lifetime")
    parser.add_argument("--hide-inactive-tracks", action="store_true", help="Do not draw inactive tracks")
    parser.add_argument("--trail-length", type=int, default=9999, help="Number of recent points drawn per track")
